...
const formatAdmission = (a) => ({
  ...
  patient: a.patient
    ? {
        ...
        childrenCount: a.patient.childrenCount,
        youngestChildAge: a.patient.youngestChildAge,
      }
    : null,
  bed: a.bed
    ? {
  ...
  ...(a.investigationOrders
    ? { pendingInvestigations: a.investigationOrders }
    : {}),
});
...
const createFullAdmission = async (req, data) => {
  return await auditedTransaction(
    req,
    { action: "CREATE", targetTable: "Admission" },
    async (tx) => {
      ...
      const doctor = await tx.user.findUnique({
        where: { id: data.admission.doctor_id },
      });
      ...
      const nurse = await tx.user.findUnique({
        where: { id: data.admission.nurse_id },
      });
      ...
      if (data.medical_history) {
        const mhData = {
          ...
          pastSimilarConditions:
            data.medical_history.past_similar_conditions || null,
          ...
          familySimilarConditions:
            data.medical_history.family_similar_conditions || null,
      ...
      if (Array.isArray(data.allergies) && data.allergies.length > 0) {
        ...
        const known = new Set(
          existing.map((a) => a.allergen.trim().toLowerCase()),
        );
      ...
      const admission = await tx.admission.create({
        data: {
          ...
          previousInvestigations:
            data.admission.previous_investigations || null,
      ...
      if (Array.isArray(data.medications) && data.medications.length > 0) {
        await tx.medication.createMany({
          data: data.medications.map((med) => ({
            ...
            frequencyText:
              med.frequency === "OTHER" ? med.frequency_text : null,
            route: med.route,
...
const dischargeAdmission = async (req, id) => {
  ...
  return auditedTransaction(
    req,
    { action: "UPDATE", targetTable: "Admission" },
    async (tx) => {
...
const archiveAdmission = async (req, id) => {
  ...
  return auditedTransaction(
    req,
    { action: "ARCHIVE", targetTable: "Admission" },
    async (tx) => {
...
const assignNurse = async (req, admissionId, data) => {
  ...
  return auditedTransaction(
    req,
    { action: "CREATE", targetTable: "AdmissionNurse" },
    async (tx) => {
...
const unassignNurse = async (req, admissionId, nurseId) => {
  ...
  return auditedTransaction(
    req,
    { action: "UPDATE", targetTable: "AdmissionNurse" },
    async (tx) => {
